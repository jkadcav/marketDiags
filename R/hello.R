...
fetchData<-function(params){
  ...
  x<-dbGetQuery(con,paste("Select meetings.id as meeting_id, events.id as event_id, event_competitors.id as event_competitor_id, competitors.id as competitor_id, trainers.id as trainer_id, venues.name as venue_name, meeting_date, countries.name as country_name, events.number as event_number, competitors.name as competitor_name, trainers.name as trainer_name,event_competitor_race_data.program_number ,event_competitor_race_data.barrier, event_competitor_race_data.finish_position, event_race_data.distance, event_race_data.race_class,
                          ...
                          (select market_json::json->'prices'->event_competitor_race_data.number-1 from markets where markets.provider = \'",markets[1],"\' and market_name = 'WIN' and markets.meeting_id = meetings.id and markets.event_number = events.number limit 1) as ",markets[1],",
                          (select market_json::json->'prices'->event_competitor_race_data.number-1 from markets where markets.provider = \'",markets[2],"\' and market_name = 'WIN' and markets.meeting_id = meetings.id and markets.event_number = events.number limit 1) as ",markets[2],",
                          (select market_json::json->'prices'->event_competitor_race_data.number-1 from markets where markets.provider = \'",markets[3],"\' and market_name = 'WIN' and markets.meeting_id = meetings.id and markets.event_number = events.number limit 1) as ",markets[3],"
                          from meetings
